 ...
 */
"use strict";
var ImageDefer=ImageDefer||{};(function(b){this.version="0.1";this.options={maxLoaded:b.maxLoaded||100,onImageRequested:b.onImageRequested||null,onImageLoaded:b.onImageLoaded||null,onImageUnloaded:b.onImageUnloaded||null,scrollingStopMillis:b.scrollingStopMillis||500,scrollingSkipRate:2};
var a={imagesLoaded:0,images:[],maxBucket:0,scrolling:false,scrollInfo:{last:{pos:0,time:0},current:{pos:0,time:0}},scrollChecker:null,lastVisibleBuckets:{start:-1,end:-1}};
...
if((d.start!==a.lastVisibleBuckets.start)||(d.end!==a.lastVisibleBuckets.end)){for(f=d.start;f<=d.end;
f++){if(a.images[f]){a.images[f].forEach(function(g){this.loadImage(g);}.bind(this));}}a.lastVisibleBuckets.start=d.start;
...
if(e<this.options.scrollingSkipRate){this.requestCallBack(this.lazyLoadImages);}}.bind(this);this.peekScroll=function(){var d=Date.now(),e=a.scrollInfo.current;
if((d-e.time>=this.options.scrollingStopMillis*0.9)&&(this.viewportPos().top===e.pos)){a.scrolling=false;
a.scrollChecker=null;this.requestCallBack(this.lazyLoadImages);}else{a.scrollChecker=setTimeout(this.peekScroll,this.options.scrollingStopMillis);
}}.bind(this);this.getBuckets=function(f,d){var e=200;return{start:Math.floor(f/e),end:Math.floor((f+d)/e)};
};this.viewportPos=function(){return{top:window.pageYOffset||document.documentElement.scrollTop||0,height:window.innerHeight||document.documentElement.offsetHeight||0};
...
}.bind(this);this.reset=function(){a.imagesLoaded=0;a.images=[];a.maxBucket=0;var d=document.querySelectorAll("img");
for(var e=0;e<d.length;e++){this.addImage(d[e]);}}.bind(this);this.addImage=function(e){var i=e.getAttribute("data-defer-src"),g=e.getAttribute("src"),d,f,h;
if(i){d=this.elementPos(e);f=this.getBuckets(d.top,d.height);for(h=f.start;h<=f.end;h++){if(!a.images[h]){a.images[h]=[];
if(h>a.maxBucket){a.maxBucket=h;}}a.images[h].push(e);}e._defer_initial_src=g;e._defer_final_src=i;e._defer_state=(this.urlsMatch(g,i)?c.loaded:c.unloaded);
if(e._defer_state===c.loaded){a.imagesLoaded++;}return true;}return false;}.bind(this);this.removeImage=function(g,i){var k=g._defer_final_src,e=g._defer_state,d,f,h,j;
...
if(e&&(e!==c.unloading)){a.imagesLoaded--;}if(i&&(!e||e===c.unloading)){g.setAttribute("src",k);}}}.bind(this);
...
};this.init=function(){document.addEventListener("scroll",this.onScroll);this.reset();this.requestCallBack(this.lazyLoadImages);
}.bind(this);}).call(ImageDefer,ImageDefer.options||{});if(ImageDefer.supportedBrowser()){if(document.readyState!=="loading"){ImageDefer.init();
}else{document.addEventListener("DOMContentLoaded",ImageDefer.init);}}else{if(console&&console.warn){console.warn("image-defer: Unsupported browser");
}}