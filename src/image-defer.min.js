...
a.resizeChecker=null;this.reset();this.requestCallBack(this.lazyLoadImages);}.bind(this),500);}.bind(this);
this.peekScroll=function(){var d=Date.now(),e=a.scrollInfo.current;if((d-e.time>=this.options.scrollingStopMillis*0.9)&&(this.viewportPos().top===e.pos)){a.scrolling=false;
...
}}.bind(this);this.addImage=function(e){var i=e.getAttribute("data-defer-src"),g=e.getAttribute("src"),d,f,h;
...
};this.init=function(){this.reset();window.addEventListener("resize",this.onResize);window.addEventListener("orientationchange",this.onResize);
document.addEventListener("scroll",this.onScroll);this.requestCallBack(this.lazyLoadImages);}.bind(this);
}).call(ImageDefer,ImageDefer.options||{});if(ImageDefer.supportedBrowser()){if(document.readyState!=="loading"){ImageDefer.init();
